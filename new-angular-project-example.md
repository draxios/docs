## Creating a brand-new Angular project with a single component—AdoProjectFormComponent—that posts JSON data to AWS API Gateway endpoint.

Project creation
Environment configuration
Service implementation (AwsApiService)
Component code (AdoProjectFormComponent)
...
Open or create src/environments/environment.ts and add the following:
...
(Optional) If you have a production file environment.prod.ts, you can set a different URL for production there.

4. Create the AWS API Service
We’ll create an Angular service that handles HTTP requests to AWS API Gateway.

In the terminal, generate a service:
...
Open aws-api.service.ts and replace its contents with:
...
This service uses HttpClient and environment variables to build the request URL. Make sure to include the correct path after your base URL (in this example: /create-ado-project).
...
Open ado-project-form.component.ts and replace its contents with:
...
Open ado-project-form.component.html and replace its contents with:
...
Optionally, open ado-project-form.component.scss if you want to add any custom styling:
...
Copy code
ng serve -o
In the browser, you’ll be redirected automatically to http://localhost:4200/.
...
Collects form data for an ADO project (project name, description, etc.).
Assembles a JSON payload including a sample repos array.
Sends that payload to an AWS API Gateway endpoint via a simple POST request (AwsApiService).
